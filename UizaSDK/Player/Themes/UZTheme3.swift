...
open class UZTheme3: UZPlayerTheme {
	...
	func setupLayout() {
		...
		let controlFrameLayout = NKGridFrameLayout(direction: .horizontal, andViews: [controlView.pipButton, controlView.airplayButton, controlView.playlistButton, controlView.ccButton, controlView.settingsButton, controlView.volumeButton])!
...
//		topFrameLayout!.showFrameDebug = true
		...
		let bottomLeftFrameLayout = NKGridFrameLayout(direction: .horizontal, andViews: [controlView.playpauseButton, controlView.currentTimeLabel])!
		let bottomRightFrameLayout = NKGridFrameLayout(direction: .horizontal, andViews: [controlView.remainTimeLabel, controlView.fullscreenButton])!
	...
	open func layoutControls(rect: CGRect) {
		...
		if let controlView = controlView {
			...
			if controlView.enlapseTimeLabel.isHidden == false {
				...
				let edgeInsets = mainFrameLayout?.bottomFrameLayout.edgeInsets ?? .zero
				controlView.enlapseTimeLabel.frame = CGRect(x: edgeInsets.left + 5, y: viewSize.height - labelSize.height - edgeInsets.bottom - 10, width: labelSize.width, height: labelSize.height)
			}
		}
	}
