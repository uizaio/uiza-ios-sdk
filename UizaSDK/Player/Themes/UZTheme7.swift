...
open class UZTheme7: UZPlayerTheme {
	...
	func setupSkin() {
		...
		let backIcon = UIImage(icon: FontType.icofont(IcofontType.close), size: iconSize, textColor: .white, backgroundColor: .clear)
		let playlistIcon = UIImage(icon: FontType.icofont(IcofontType.listineDots), size: iconSize, textColor: iconColor, backgroundColor: .clear)
		let helpIcon = UIImage(icon: FontType.icofont(IcofontType.questionCircle), size: iconSize, textColor: iconColor, backgroundColor: .clear)
		let ccIcon = UIImage(icon: FontType.icofont(IcofontType.cc), size: iconSize, textColor: iconColor, backgroundColor: .clear)
		let settingsIcon = UIImage(icon: FontType.icofont(IcofontType.gear), size: iconSize, textColor: iconColor, backgroundColor: .clear)
		let volumeIcon = UIImage(icon: FontType.icofont(IcofontType.volumeUp), size: iconSize, textColor: iconColor, backgroundColor: .clear)
		let muteIcon = UIImage(icon: FontType.icofont(IcofontType.volumeMute), size: iconSize, textColor: iconColor, backgroundColor: .clear)
...
//		let pauseBigIcon = UIImage(icon: .googleMaterialDesign(.pauseCircleOutline), size: centerIconSize, textColor: iconColor, backgroundColor: .clear)
		let playIcon = UIImage(icon: FontType.openIconic(.playCircle), size: centerIconSize, textColor: iconColor, backgroundColor: .clear)
		let pauseIcon = UIImage(icon: FontType.openIconic(.mediaPause), size: centerIconSize, textColor: iconColor, backgroundColor: .clear)
		...
		let forwardIcon = UIImage(icon: FontType.googleMaterialDesign(.fastForward), size: CGSize(width: 32, height: 32), textColor: iconColor, backgroundColor: .clear)
		let backwardIcon = UIImage(icon: FontType.googleMaterialDesign(.fastRewind), size: CGSize(width: 32, height: 32), textColor: iconColor, backgroundColor: .clear)
		let nextIcon = UIImage(icon: FontType.googleMaterialDesign(.skipNext), size: iconSize, textColor: iconColor, backgroundColor: .clear)
		let previousIcon = UIImage(icon: FontType.googleMaterialDesign(.skipPrevious), size: iconSize, textColor: iconColor, backgroundColor: .clear)
		let thumbIcon = UIImage(icon: FontType.fontAwesomeSolid(.circle), size: seekThumbSize, textColor: .white, backgroundColor: .clear)
...
//		topFrameLayout!.showFrameDebug = true
		...
		bottomFrameLayout = StackFrameLayout(axis: .horizontal, views: [bottomLeftFrameLayout, bottomCenterFrameLayout, bottomRightFrameLayout])
...
//		bottomFrameLayout!.backgroundColor = UIColor(white: 0.0, alpha: 0.8)
		...
		mainFrameLayout = StackFrameLayout(axis: .vertical) // views: [topFrameLayout!, playpauseCenterButton, bottomFrameLayout!]
